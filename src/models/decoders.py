...
class ViterbiAlgorithm:
    ...
    def best_hidden_state_sequence(self, decode_observation_states: np.ndarray) -> np.ndarray:
        ...
        delta = np.reshape(delta[:, first_obs_index], (-1, 1))  # decided to not scale delta
        ...
        for trellis_node in range(1, len(decode_observation_states)):
            ...
            # Update delta; decided not to scale
        ...
        best_hidden_state_path = np.vectorize(self.hmm_object.hidden_states_dict.get) \
